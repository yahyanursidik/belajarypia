...
async function run() {
  const { data: participants, error: pError } = await supabase.from('participants').select('user_id').limit(1);
  if (pError || !participants || participants.length === 0) {
    console.error('Failed to fetch participant', pError);
    return;
  }
  const uid = participants[0].user_id;
  
  console.log('Attempting to delete REAL user:', uid);
